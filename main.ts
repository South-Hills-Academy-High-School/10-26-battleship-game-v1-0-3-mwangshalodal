...
function isPlayerXWinner (enmyBoats: Sprite[][], hitOrMissPX: Sprite[]) {
    killCount = 0
    for (let index = 0; index <= 2; index++) {
        currentBoatCounter = 0
        for (let currentBoatSprite of enmyBoats[index]) {
            for (let currentBoomSprite of hitOrMissPX) {
                if (grid.spriteRow(currentBoomSprite) == grid.spriteRow(currentBoatSprite) && grid.spriteCol(currentBoomSprite) == grid.spriteCol(currentBoatSprite)) {
                    currentBoatCounter += 1
                    break;
                }
            }
        }
        if (currentBoatCounter == enmyBoats[index].length) {
            killCount += 1
        }
    }
    return killCount
    ...
    for (let index = 0; index <= 2; index++) {
        for (let currentBoatSprite3 of enemyBoats[index]) {
            if (grid.spriteCol(currentBoatSprite3) == grid.spriteCol(cursor) && grid.spriteRow(currentBoatSprite3) == grid.spriteRow(cursor)) {
                ...
                game.splash("" + currentPlayer + "   HIT!!" + convertToText(isPlayerXWinner(enemyBoats, hitOrMissPX)))
                return 1
            }
        }
    }
